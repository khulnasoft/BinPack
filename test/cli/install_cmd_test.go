			...
			steps: []step{
				{
					...
					assertions: []traitAssertion{
						...
						assertFileInStoreExists(".binpack.state.json"),
						assertFileInStoreExists("binpack"),
						assertManagedToolOutput("binpack", []string{"--version"}, "binpack v0.7.0\n"),
					},
				},
				{
					name: "list",
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, s := range test.steps {
				t.Run(s.name, func(t *testing.T) {
					...
					s.env["BINPACK_ROOT"] = d

					cmd, stdout, stderr := runBinpack(t, s.env, s.args...)
