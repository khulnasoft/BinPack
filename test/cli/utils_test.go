...
func runBinpack(t testing.TB, env map[string]string, args ...string) (*exec.Cmd, string, string) {
	return runBinpackCommand(t, env, true, args...)
}

func runBinpackCommand(t testing.TB, env map[string]string, expectError bool, args ...string) (*exec.Cmd, string, string) {
	cancel := make(chan bool, 1)
	defer func() {
		cancel <- true
	}()

	cmd := getBinpackCommand(t, args...)
	if env == nil {
	...
	if !expectError && err != nil && stdout == "" {
		t.Errorf("error running binpack: %+v", err)
		...
		if err != nil {
			t.Errorf("error rerunning binpack: %+v", err)
			t.Errorf("STDOUT: %s", stdout)
			t.Errorf("STDERR: %s", stderr)
		}
	}
...
func getBinpackCommand(t testing.TB, args ...string) *exec.Cmd {
	return exec.Command(getBinaryLocation(t), args...)
}

func getBinaryLocation(t testing.TB) string {
	if os.Getenv("BINPACK_BINARY_LOCATION") != "" {
		// BINPACK_BINARY_LOCATION is the absolute path to the snapshot binary
		return os.Getenv("BINPACK_BINARY_LOCATION")
	}
...
func getBinaryLocationByOS(t testing.TB, goOS string) string {
	...
	switch goOS {
	case "darwin", "linux":
		return path.Join(repoRoot(t), fmt.Sprintf("snapshot/%s-build_%s_%s/binpack", goOS, goOS, archPath))
	default:
