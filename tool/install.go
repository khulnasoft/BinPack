...
func Install(tool binpack.Tool, intent binpack.VersionIntent, store *binpack.Store, verifyConfig VerifyConfig) (err error) {
	...
	if errors.Is(err, binpack.ErrMultipleInstallations) {
		return err
	}
	if err == nil {
...
func makeStagingArea(store *binpack.Store, toolName string) (string, func(), error) {
	cleanup := func() {}
	...
	tmpdir, err := os.MkdirTemp(absRoot, fmt.Sprintf("binpack-install-%s-", toolName))
	if err != nil {
